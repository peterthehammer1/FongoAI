...
/**
 * Script to update the existing Fongo CC updater agent using the SDK
 */

async function updateFongoAgentSDK() {
  try {
    ...
    console.log('🔄 Updating existing Fongo CC updater agent using SDK...');
    ...
    // Update the agent using SDK
    const response = await retell.updateAgent(existingAgentId, updatePayload);

    console.log('✅ Agent updated successfully!');
    console.log('Updated Agent ID:', response.agent_id);
    console.log('Agent Name:', response.agent_name);
    console.log('Webhook URL:', response.webhook_url);
    console.log('WebSocket URL:', response.response_engine.llm_websocket_url);
    ...
    if (envContent.includes('RETELL_AGENT_ID=')) {
      envContent = envContent.replace(/RETELL_AGENT_ID=.*/, `RETELL_AGENT_ID=${response.agent_id}`);
    } else {
      envContent += `\nRETELL_AGENT_ID=${response.agent_id}\n`;
    }
...
module.exports = { updateFongoAgentSDK };
